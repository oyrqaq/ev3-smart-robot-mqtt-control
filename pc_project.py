...
def main():
    ...
    calibrate_button.grid(row=5, column=1)
    calibrate_button['command'] = lambda: send_calibrate(mqtt_client)
    root.bind('<h>', lambda event: send_calibrate(mqtt_client))

    line_follow_button = ttk.Button(main_frame, text="Follow Line")
    line_follow_button.grid(row=6, column=1)
    line_follow_button['command'] = lambda: send_follow_line(mqtt_client)
...
def send_calibrate(mqtt_client):
    print("arm_calibration")
    mqtt_client.send_message("arm_calibration")

def send_follow_line(mqtt_client):
    print("follow_line")
